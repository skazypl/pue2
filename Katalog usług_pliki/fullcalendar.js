...
//                        week: "MMMM d[ yyyy]{ '&#8212;'[ MMMM] d yyyy}",
	...
	firstHour: 8,
	slotMinutes: 30,
	minSlotMinutesSize: 5,
	maxSlotMinutesSize: 60,
	newSlotMinutesSize: null,
...
$.fn.fullCalendar = function(options) {
	...
	// Pobiera ze wskazanego url domyslna konfiguracje kalendarza
//                if(options.configURL){
//                        var servConfig = getServerConfig(options.configURL);
//                        // Nadpisuje domyslne ustawienia znajdujace sie w tym pliku - zmienna defaults
//                        setDefaults(servConfig);
//                }
...
-----------------------------------------------------------------------------*/
	...
	function changeView(newViewName, force) {
		...
		if (!currentView || newViewName != currentView.name || force == true) {
			...
			renderView(undefined, force); // after height has been set, will make absoluteViewElement's position=relative, then set to null
...
function renderAgain() {
	...
	function renderView(inc, optionsChanged) {
		/* ATutor
		...
		if (elementVisible()) {
			...
			updateEvents(forceEventRender, optionsChanged);
...
-----------------------------------------------------------------------------*/
	...
	function updateEvents(forceRender, optionsChanged) {
		if(optionsChanged){
			rerenderEvents();
		}else{
	...
	function calculateSlotSize(events){
		var slotSizeCalc = new SlotSizeCalc();
		slotSizeCalc.aktualSlotMinutesSize = t.option("slotMinutes");
		slotSizeCalc.minSlotMinutesSize = t.option("minSlotMinutesSize");
		slotSizeCalc.maxSlotMinutesSize = t.option("maxSlotMinutesSize");
		slotSizeCalc.newSlotMinutesSize = t.option("newSlotMinutesSize");
		var wynik = slotSizeCalc.computeSlotSize(events);
		if(wynik){
			t.option("slotMinutes", wynik);
		}
	}
	
			function SlotSizeCalc() {
				this.minSlotMinutesSize = 5;
				this.maxSlotMinutesSize = 60;
				this.aktualSlotMinutesSize = 30;
				this.newSlotMinutesSize = null;
				this._constructor = function(){
				}
				this.dateMinuteDifference = function(start, end){
					//                                        return dojo.date.difference(new Date(start), new Date(end), "minute");
					var startDate = new Date(start);
					var endDate = new Date(end);
					var diff = new Date(endDate - startDate);
					return diff/1000/60;
				}
				this.findMinEvent = function(array){
					var min = null;
					if(array){
						var _this = this;
						$.each(array, function(i, entry){
							var slotSize = _this.dateMinuteDifference(new Date(entry.start), new Date(entry.end));
							if(min){
								if(slotSize < min){
									min = slotSize;
								}
				...
				this.computeSlotSize = function(events){
					console.log("----------------------------------");
					console.log("computeSlotSize [aktualSlotMinutesSize = " + this.aktualSlotMinutesSize + ", minSlotMinutesSize = " + this.minSlotMinutesSize + ", maxSlotMinutesSize = " + this.maxSlotMinutesSize + ", newSlotMinutesSize = " + this.newSlotMinutesSize);
					var minEventSlot = null;
					if($.isArray(events)){
						minEventSlot = this.findMinEvent(events);
					}
					console.log("computeSlotSize: minEventSlot = " + minEventSlot);
					if(this.newSlotMinutesSize && (this.newSlotMinutesSize < minEventSlot || minEventSlot == null )){
						minEventSlot = this.newSlotMinutesSize;
					}
					if(minEventSlot == null){
						minEventSlot = this.aktualSlotMinutesSize;
					}
					minEventSlot = Math.max(minEventSlot, this.minSlotMinutesSize);
					minEventSlot = Math.min(minEventSlot, this.maxSlotMinutesSize);

					if(minEventSlot >= this.minSlotMinutesSize
						&& minEventSlot <= this.maxSlotMinutesSize
						&& minEventSlot != this.aktualSlotMinutesSize){
						//  console.log("Zmieniam wielkosc slotu na: " + minEventSlot);
						console.log("computeSlotSize -> " + minEventSlot );
						return minEventSlot;
					}else{
						console.log("computeSlotSize -> " + null );
						// console.log("NIE zmieniono wielkosc slotu");
						return null;
					}
				}
				this._constructor();
...
-----------------------------------------------------------------------------*/
	...
	function option(name, value) {
	   ...
	   renderAgain();
		...
		if(name == 'newSlotMinutesSize' && typeof value == 'number') {
	   options[name] = value;
	}
	}
...
------------------------------------------------------------------------------*/
	...
	function normalizeSource(source) {
		...
		//calculateSlotSize(source.events);
	}
	...
	function buildSkeleton() {
		...
		for (i=0; d < maxd; i++) {
			...
			for (zz = 0; zz < colCnt; zz++) {
				...
				"<td  class='" + contentClass + "'><a class='fc-dummy' ><div class='fc-cell-time'>" + formatDate(d, opt('axisFormat')) + " </div><div class='fc-cell-id'></div><div class='fc-cell-date'>" + /*(date.getDate() + " " + monthNames[date.getMonth()] + " " + date.getFullYear()) +*/ "</div><div class='fc-month-vevent'></div>" + // fc- needed for setDayID
			"<div style='position:relative'>&nbsp;</div>" +
				"</a></td>";
			}
...
----------------------------------------------------------------------------*/
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			seg = segs[i];
			if (eventElement = seg.element) {
				eventElement[0].style.width = Math.max(0, seg.outerWidth + seg.hsides + 2) + 'px';
	...
	function slotSegHtml(event, seg) {
		...
		"<div class='fc-event-time'> godzina " +
