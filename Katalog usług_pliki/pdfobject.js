...
var PDFObject = function (obj){
	...
	var pdfobjectversion = "1.2",
		...
		id = obj.id || false,
		width = obj.width || "100%",
		height = obj.height || "99%",
		pdfOpenParams = obj.pdfOpenParams,
		url,
		pluginTypeFound,
		parameterSeparator = "?",
		zoomParameterSeparator = "#",
		zoomParameter = "zoom=100",
		missingPluginInfo = "Proszę zainstalować wtyczkę firmy Adobe umożliwiającą odczytywanie plików PDF",
		downloadPdfInfo = "Kliknij aby pobrać plik PDF";
	...
	var hasReaderActiveX = function (){
		...
		if (window.ActiveXObject) {
			...
			//If "AcroPDF.PDF" found, return true
			if (axObj !== null) {
				return true;
			}
			...
			//If "PDF.PdfCtrl" found, return true
			if (axObj !== null) {
				return true;
			}
	...
	var buildQueryString = function(pdfParams){
		...
		for (prop in pdfParams) {
			...
			if (pdfParams.hasOwnProperty(prop)) {
				...
				string += prop;
				string += "=";
				string += encodeURIComponent(pdfParams[prop]);
				string += "&";
			}
		}
		...
		// ... and add zoom
		string += zoomParameterSeparator + zoomParameter;
	...
	var embed = function(node){
		...
		if(!pluginTypeFound){
			node.innerHTML = '<div style="padding-left:30px;padding-top:30px"><b>' + missingPluginInfo + '</b></div>';
			node.innerHTML += '<a style="padding-left:30px;cursor:pointer">'+ downloadPdfInfo + '</a>';
			return node.getElementsByTagName("a")[0];
		}
		
		node.innerHTML = '<object data="' +url +'" type="application/pdf" width="' +width +'" height="' +height +'"></object>';
		return node.getElementsByTagName("object")[0];
	};

	//The hash (#) prevents odd behavior in Windows
	//Append optional Adobe params for opening document
	url = encodeURI(obj.url) + parameterSeparator + buildQueryString(pdfOpenParams);
